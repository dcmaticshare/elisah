...
class KadenceGradientColorPicker extends Component {
	...
	render() {
		const getColorValue = () => {
			let color;
			if ( undefined !== this.state.color && '' !== this.state.color && isString( this.state.color ) && this.state.color.includes( 'palette' ) ) {
				color = this.props.activePalette[ parseInt( this.state.color.slice(-1), 10 ) - 1 ].color;
			} else {
		...
		return (
			<div className="kadence-background-color-wrap">
				...
				<SwatchesControl
					...
					isPalette={ ( undefined !== this.state.color && '' !== this.state.color && isString( this.state.color ) && this.state.color.includes( 'palette' ) ? this.state.color : '' ) }
